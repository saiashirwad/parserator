...
describe("scheme", () => {
  it("should parse a simple expression", () => {
    ...
    expect(result).toEqual(
      LispExpr.list([LispExpr.symbol("+"), LispExpr.number(1), LispExpr.number(2)])
    );
  });
  ...
  it("should parse a nested expression", () => {
    ...
    expect(result).toEqual(
      LispExpr.list([
        LispExpr.symbol("+"),
        LispExpr.number(1),
        LispExpr.list([LispExpr.symbol("+"), LispExpr.number(2), LispExpr.number(3)])
      ])
    );
  });
  ...
  it("should parse a list", () => {
    const result = expr.parseOrThrow("(1 2 3)");
    expect(result).toEqual(
      LispExpr.list([LispExpr.number(1), LispExpr.number(2), LispExpr.number(3)])
    );
  });
  ...
  it("should parse a let expression with multiple bindings", () => {
    ...
    expect(result).toEqual(
      LispExpr.let(
        ...
        LispExpr.list([LispExpr.symbol("+"), LispExpr.symbol("x"), LispExpr.symbol("y")])
      )
    );
  });
  ...
  it("should parse a lambda expression", () => {
    ...
    expect(result).toEqual(
      LispExpr.lambda(
        ["x"],
        LispExpr.list([LispExpr.symbol("+"), LispExpr.symbol("x"), LispExpr.number(2)])
      )
    );
  });
  ...
  it("should parse a let expression with a lambda expression", () => {
    ...
    expect(result).toEqual(
      LispExpr.let(
        [{ name: "x", value: LispExpr.lambda(["y"], LispExpr.symbol("y")) }],
  ...
  it("should parse a list", () => {
    ...
    expect(result).toEqual(
      LispExpr.list([LispExpr.symbol("a"), LispExpr.symbol("b"), LispExpr.symbol("c")])
    );
  });
  ...
  it("should parse a let expression with a single binding", () => {
    ...
    expect(result).toEqual(
      LispExpr.let([{ name: "x", value: LispExpr.number(1) }], LispExpr.symbol("x"))
    );
  });
});
