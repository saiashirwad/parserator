...
function parseJSDoc(comment: string): JSDocComment {
  ...
  let currentSection: 'description' | 'param' | 'returns' | 'example' | 'template' = 'description';
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmedLine.startsWith('@param')) {
      ...
      const match = trimmedLine.match(/@param\s+(?:\{([^}]+)\}\s+)?(\w+)(?:\s+-\s+(.*))?/);
      if (match) {
        jsdoc.params.push({
          name: match[2],
      ...
      const templates = trimmedLine.replace('@template', '').trim().split(',').map(t => t.trim());
      ...
      if (currentSection === 'description') {
        ...
        lastParam.description += (lastParam.description ? ' ' : '') + trimmedLine;
      } else if (currentSection === 'returns' && jsdoc.returns) {
        jsdoc.returns.description += (jsdoc.returns.description ? ' ' : '') + trimmedLine;
      }
    } else if (currentSection === 'example') {
...
function extractCompleteSignature(content: string, startIndex: number): string {
  ...
  for (let i = startIndex; i < content.length; i++) {
    ...
    if (!inString && !inTemplate && depth === 0 && (char === ')' || char === '}')) {
 ...
 */
function parseParameter(param: string): { name: string; type: string; optional?: boolean } | null {
 ...
 */
function extractFromTypeScript(content: string, filePath: string): { functions: ParsedFunction[], classes: ParsedClass[] } {
  ...
  while ((match = jsdocPattern.exec(content)) !== null) {
    ...
    // Skip whitespace
    while (declarationStart < content.length && /\s/.test(content[declarationStart])) {
      declarationStart++;
    }
 ...
 */
async function processFile(filePath: string): Promise<{ functions: ParsedFunction[], classes: ParsedClass[] }> {
...
function generateFunctionDoc(func: ParsedFunction): string {
  ...
  const params = func.extractedTypes?.params || func.jsdoc.params.map(p => ({
    name: p.name,
  ...
  const returnType = func.extractedTypes?.returnType || 
    (func.jsdoc.returns?.type !== 'unknown' ? func.jsdoc.returns?.type : undefined);
...
async function generateAPIDocs() {
  ...
  const moduleMap = new Map<string, { functions: ParsedFunction[], classes: ParsedClass[] }>();
