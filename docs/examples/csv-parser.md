...
function createCsvParser(config: Partial<CsvConfig> = {}) {
  ...
  // Line terminators
  const lineEnd =
    Array.isArray(cfg.lineTerminator) ?
  ...
  const quotedContent = many0(or(escapedChar, notChar(cfg.quote))).map(chars =>
    chars.join("")
  );

  // Quoted field
  const quotedField = between(char(cfg.quote), char(cfg.quote), quotedContent);

  // Unquoted field (until delimiter or line end)
  const unquotedField = takeUntil(or(char(cfg.delimiter), lineEnd, eof));
  ...
  const line = or(row, cfg.skipEmptyLines ? emptyLine : row.map(() => []));
  ...
  feed(chunk: string): Array<string[] | Record<string, string>> {
    ...
    while (true) {
      ...
      this.buffer = this.buffer.slice(
        lineEndIndex + this.config.lineTerminator.length
      );
  ...
  validate(data: Record<string, string>[], headers: string[]): any {
    ...
    data.forEach((row, rowIndex) => {
      this.rules.forEach(rule => {
        const columnName =
          typeof rule.column === "number" ? headers[rule.column] : rule.column;
