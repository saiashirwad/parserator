...
```typescript
const parser = string("hello").withError(() => "Expected a greeting");

const parser2 = many1(digit).withError(
  ({ state }) => `Expected a number at position ${state.pos.offset}`
);
```
...
const factor = or(
  ...
  char("(")
    .then(Parser.lazy(() => expression))
    .thenDiscard(char(")")),
  // Number
  many1(digit).map(d => parseInt(d.join("")))
);

const term = sepBy1(char("*"), factor).map(factors => factors.reduce((a, b) => a * b));

expression = sepBy1(char("+"), term).map(terms => terms.reduce((a, b) => a + b));
```
...
// Use with context
const result = parser.parse("input", { debug: true, variables: new Map(), source: "input" });
```
